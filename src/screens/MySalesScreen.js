...
export default function MySalesScreen({ navigation }) {
  ...
  const generateMockSales = (pageNum) => {
    ...
    for (let i = 0; i < itemsPerPage; i++) {
      ...
      const buyerRating = Math.floor(Math.random() * 5) + 1;
      const saleDate = new Date(Date.now() - index * 24 * 60 * 60 * 1000);
      const listDate = new Date(saleDate.getTime() - 10 * 24 * 60 * 60 * 1000);

      salesData.push({
        ...baseProduct,
        id: `sale_${pageNum}_${i}`,
  ...
  const renderSaleItem = ({ item }) => (
    <TouchableOpacity
      style={[styles.saleCard, { width: CARD_WIDTH }]}
  ...
  const renderEmptyState = () => {
    ...
    return (
      <View style={styles.emptyState}>
        <BoxBold color={colors.textSecondary} size={64} />
        <Text style={[styles.emptyTitle, { color: colors.text }]}>Aucune vente</Text>
        <Text style={[styles.emptySubtitle, { color: colors.textSecondary }]}>
          Vous n'avez pas encore vendu d'article.
        </Text>
      </View>
    );
  };
  ...
  const getSalesCount = () => {
    ...
    return `${filteredSales.length} vente${filteredSales.length > 1 ? 's' : ''}`;
  };
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.headerBg, borderBottomColor: colors.border, paddingTop: insets.top }]}>
        ...
        <View style={styles.headerCenter}>
          <Text style={[styles.headerTitle, { color: colors.text }]}>Mes ventes</Text>
        </View>
                      ...
                      >
                        ...
                        <View style={styles.buyerDetails}>
                          ...
                          <View style={styles.buyerRating}>
                            <StarSolid color="#FFB800" size={14} />
                            <Text style={[styles.buyerRatingText, { color: colors.text }]}>
                              {selectedProduct.buyer.rating}
                            </Text>
                          </View>
                        </View>
                      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  buyerDetails: { flex: 1, gap: 4 },
  buyerNameRow: { flexDirection: 'row', alignItems: 'center', gap: 4 },

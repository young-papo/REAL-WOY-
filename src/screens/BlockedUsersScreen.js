...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, Image, StatusBar, useColorScheme, Alert, RefreshControl } from 'react-native';
...
export default function BlockedUsersScreen({ navigation }) {
  ...
  const loadBlockedUsers = async () => {
    ...
    try {
      ...
      const mockBlockedUsers = [
        { id: 'b1', oderId: 'user_b1', username: 'spam_account', avatar: 'https://i.pravatar.cc/150?img=10', verified: false },
        { id: 'b2', oderId: 'user_b2', username: 'toxic_user', avatar: null, verified: false },
        { id: 'b3', oderId: 'user_b3', username: 'annoying_person', avatar: 'https://i.pravatar.cc/150?img=25', verified: true },
        { id: 'b4', oderId: 'user_b4', username: 'fake_profile', avatar: 'https://i.pravatar.cc/150?img=35', verified: false },
        { id: 'b5', oderId: 'user_b5', username: 'blocked_seller', avatar: null, verified: true },
        { id: 'b6', oderId: 'user_b6', username: 'verified_scammer', avatar: 'https://i.pravatar.cc/150?img=40', verified: true },
      ];
  ...
  const handleUnblock = (user) => {
    Alert.alert(
      ...
      `Voulez-vous vraiment débloquer ${user.username} ?`,
      [
  ...
  const handleUserPress = () => {
    Alert.alert('Accès limité', 'L\'accès est limité. Débloquez l\'utilisateur pour voir son profil.');
  };
  ...
  const renderUserItem = (user) => (
    <TouchableOpacity key={user.id} style={styles.userItem} onPress={handleUserPress} activeOpacity={0.7}>
      {user.avatar ? (
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={[styles.emptyText, { color: colors.emptyIcon }]}>Vous n'avez bloqué personne</Text>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  userAvatar: { width: 50, height: 50, borderRadius: 25, backgroundColor: '#E0E0E0', marginRight: 12 },
  userAvatarPlaceholder: { width: 50, height: 50, borderRadius: 25, marginRight: 12, alignItems: 'center', justifyContent: 'center' },
  ...
  emptyContainer: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingVertical: 100, paddingHorizontal: 32 },
  emptyText: { fontSize: 14, textAlign: 'center', marginTop: 16, lineHeight: 20 },
  skeletonContainer: { paddingTop: 8 },
  skeletonItem: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12 },
